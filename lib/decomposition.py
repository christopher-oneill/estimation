...
def POD(fluctuating_velocity):
    # we assume that the dimension order is (x,t,c) or (x,t)
    ...
    assert(np.abs(0.5*np.sum(np.power(A,2.0),axis=(0,1))/nt - 0.5*np.trace(C))<1E-6)
    if not np.abs(0.5*np.trace(C) - 0.5*np.sum(Lambda,axis=0))<(1E-4*(1/nt)):
        print(np.abs(0.5*np.trace(C) - 0.5*np.sum(Lambda,axis=0)))
        raise ValueError("Energy not converged")
