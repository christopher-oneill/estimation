...
master_pod_path = os.path.join(base_dir,"strip10_stride1_field_only", "mp_pod_master_raw_sensor.h5") # Your pre-computed file
model_dir = os.path.join(base_dir,"strip10_stride1_field_only", f"Model_BiLSTM_{MODEL_NUMBER}")
...
# Hyperparameters
config = {
    'n_modes_ss': 2*363,   # Input: First 33 sensor modes (Observable limit)
...
def train():
    ...
    with h5py.File(master_pod_path, 'r') as f:
        ...
        a_ss_tr = f['train/s_raw'][:, :config['n_modes_ss']]
        a_ff_ts = f['test/a_ff'][:, :config['n_modes_ff']]
        a_ss_ts = f['test/s_raw'][:, :config['n_modes_ss']]
    ...
    if os.path.isfile(ckpt_latest):
        print(f"Loading checkpoint from {ckpt_latest}...")
