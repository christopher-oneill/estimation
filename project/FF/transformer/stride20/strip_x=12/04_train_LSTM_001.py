...
model_dir = os.path.join(output_dir, f"Model_{MODEL_NUM}_FF")
...
class SupersamplingBiLSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_layers=3):
        super().__init__()
        # 3 layers for deeper temporal extraction of chaotic features
    ...
    def forward(self, x):
        # Extract features from the final hidden state of the Bi-LSTM
        out, _ = self.lstm(x)
        # Use the centered time-step (last output of the Bi-LSTM layers)
...
def train():
    ...
    model = SupersamplingBiLSTM(input_dim, HIDDEN_DIM, output_dim).to(device)
    ...
    for epoch in range(start_epoch, 1000):
        model.train()
        train_loss = 0
        for bx, by in tqdm(train_loader, desc=f"Epoch {epoch}", leave=False):
