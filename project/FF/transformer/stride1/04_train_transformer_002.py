...
# --- 1. CONFIGURATION ---
MODEL_NUM = 1
output_dir = "/media/chris-remote/Projects/ONeill/estimation/output/kevin_2cylinder/stride1"
...
def train():
    ...
    train_ds = SparseLabelDataset(scaler_ss.transform(a_ss_train), 
                                  scaler_ff.transform(a_ff_train), config['seq_len'], config['stride'])
    val_ds = SparseLabelDataset(scaler_ss.transform(a_ss_test), 
                                scaler_ff.transform(a_ff_test), config['seq_len'], config['stride'])
    ...
    for epoch in range(start_epoch, start_epoch+4000):
        # --- NOISE SCHEDULER ---
        initial_noise = 0.02
        final_noise = 0.0005  # A tiny bit of "regularization floor"
